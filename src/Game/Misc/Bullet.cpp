...
Bullet::Bullet(std::string s_Name_In, Mesh_3D * p_Mesh_In, GameObject_3D * parentObj, Texture * diffuse, Texture * specular)
{
	...
	if (parentObj->get_Children().find("Robot Left Arm")->second)
	{
		set_Position(parentObj->get_Position() + static_cast<GameObject_3D*>(parentObj->get_Children().find("Robot Left Arm")->second)->get_Position());
